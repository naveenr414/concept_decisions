...
def policy_coverage_selection_multiple(
    ground_truth_gym_env,
    concept_list,
    num_concepts_selected,
    groundtruth_model,
    ...
    for i, (_, elems) in enumerate(final_vals):
        if elems:  # make sure not empty
            model.addConstr(
                y_2[i] <= gp.quicksum(x[e]*(acc_list[e]**2+(1-acc_list[e])**2) for e in elems),
                name=f"cover_{i}"
            )
        else:
            model.addConstr(y_2[i] == 0)  # cannot be covered
    ...
    if model.Status not in (GRB.OPTIMAL, GRB.TIME_LIMIT):
        ...
        if coverage_ratio < 0:
            ...
            return policy_coverage_selection_multiple(
                ground_truth_gym_env,
                concept_list,
                num_concepts_selected,
                groundtruth_model,
                q_estimates,acc_list,
                coverage_ratio=coverage_ratio
            )
    ...
    if len_x_vals < num_concepts_selected and fixed_idx == []:
        return policy_coverage_selection_multiple(ground_truth_gym_env,concept_list,
                                                   num_concepts_selected,groundtruth_model,
...
def policy_coverage_selection_lp_unweighted(
    ground_truth_gym_env,
    concept_list,
    num_concepts_selected,
    groundtruth_model,
...
def policy_coverage_selection_lp_hybrid_prefix(
    ground_truth_gym_env,
    concept_list,
    num_concepts_selected,
    groundtruth_model,
...
def policy_coverage_selection_lp_hybrid_on_restart(
    ground_truth_gym_env,
    concept_list,
    num_concepts_selected,
    groundtruth_model,
    ...
    if len(fixed_idx) == 0:
        ...
        y_2 = model.addVars(len(final_vals), lb=0.0, vtype=GRB.BINARY, name="y_2")
