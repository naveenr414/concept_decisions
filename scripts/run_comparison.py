...
if __name__ == "__main__":
    ...
    from concept_abstraction.training import (
        train_ppo, evaluate_model, ConceptPredictorCNN,
    )
    from concept_abstraction.selection import (
        random_selection, variance_selection, greedy_selection, drs, drs_log,
    )
    ...
    if setting == "perfect" and method == "drs_log":
        raise ValueError(
            "drs_log requires a trained concept predictor; "
            "use --setting imperfect or intervention.")
    ...
    if setting in ("imperfect", "intervention"):
        ...
        predictor_name = REPO_ROOT / "results/models/concept_predictor_env={}_training={}_seed={}.pth".format(
        ...
        concept_predictor = ConceptPredictorCNN(
            len(concept_list), num_frames=num_frames,
            height=height, width=width).to(device)
        concept_predictor.load_state_dict(
            torch.load(predictor_name, weights_only=True))
        ...
        if method == "drs_log":
            if setting == "intervention":
                ...
                if matching and "concept_accuracy" in matching[0]:
                    ...
                    # Fall back to evaluating the predictor directly
                    acc_list = _evaluate_predictor_accuracy(
                        concept_predictor, ground_truth_gym_env,
                        groundtruth_model, concept_list,
                        num_frames, height, width, device)
        ...
        if torch.cuda.is_available():
            ...
            with torch.no_grad():
                _ = concept_predictor(
                    torch.zeros(8, num_frames, height, width, device="cuda"))
    ...
    if method == "all_concepts":
        ...
        _, idx = random_selection(concept_list, num_concepts_selected)
    ...
    if setting == "perfect":
        ...
        env_kwargs = dict(
            fast_predictor=concept_predictor,
            use_processed=True,
            concept_idx=idx,
    ...
    model = train_ppo(
        two_stage_env, environment_string, seed=seed, policy="MlpPolicy",
