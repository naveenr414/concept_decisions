...
do 
  ...
  tmux send-keys -t concept_iterative "conda activate ${environment}; python -u main_experiments.py --seed ${seed} --environment_string ${env} --training_timesteps ${training} --gold_timesteps 10000000 --num_iterations ${num_iterations} --selections_per_round ${num_concepts_selected} --selection_function q_value --out_folder iterative --concept_source human_selected_binary --cbm_accuracy_by_concept ${cbm_accuracy} --run_iterative >> ${LOGFILE} 2>&1"  ENTER 
done 
