...
METHODS=(policy_selection_lp policy_selection_td policy_selection_multiple) #(imperfect_concepts random entropy greedy lp multiple)
...
for i in "${!SEEDS[@]}"; do
  ...
  for m in "${METHODS[@]}"; do
    session_name="imperfect_cart_pole_pong_${m}_${seed}"
  ...
  for m in "${METHODS[@]}"; do
    session_name="imperfect_mini_grid_boxing_${m}_${seed}"
    setup_tmux_session "$session_name" "$gpu"
    : > "runs/logs/error_${session_name}.txt"
  done
