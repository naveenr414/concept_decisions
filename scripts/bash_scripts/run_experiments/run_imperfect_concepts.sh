...
for idx in "${!SEEDS[@]}"; do
  ...
  for concept_fraction in 0.8 1.2 1.6 
  do 
    for method in random entropy greedy lp_hybrid 
    do 
      for env in cart_pole mini_grid pong boxing glucose 
      do 
        session_name="perfect_${env}_${method}_${concept_fraction}_seed${seed}"
        setup_tmux_session "$session_name" "$gpu"
        : > "runs/logs/error_${session_name}.txt"
      done 
    done 
...
for seed in "${SEEDS[@]}"; do
  ...
  for env in glucose mini_grid cart_pole
  do
  ...
  for env in cart_pole mini_grid pong boxing glucose
  do 
    for concept_fraction in 0.8 1.2 1.6; do
      for method in random entropy greedy lp_hybrid 
      do 
        tmux_target="perfect_${env}_${method}_${concept_fraction}_seed${seed}"
