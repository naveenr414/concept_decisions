...
Runs three evaluations in a single invocation:
  ...
  (perfect concepts are not evaluated here — CUB has no RL environment)
...
def evaluate(selected_concepts, eval_X, hidden_size=256):
    ...
    mlp = MLPClassifier(hidden_layer_sizes=(hidden_size,), activation='relu', solver='adam')
...
for c in num_concepts_range:
    concepts = basic_greedy_selection_supervised(train_X, train_Y, c)
...
for c in num_concepts_range:
    concepts = lp_selection_supervised(train_X, train_Y, c)
...
for c in num_concepts_range:
    concepts = multiple_log_selection_supervised(train_X, train_Y, train_concept_accuracy, c)
...
selections = {
    ...
    'variance': basic_greedy_selection_supervised(train_X, train_Y, k),
    'greedy':   greedy_selection_supervised(train_X, train_Y, k),
    'drs':      lp_selection_supervised(train_X, train_Y, k),
    'drs_log':  multiple_log_selection_supervised(train_X, train_Y, train_concept_accuracy, k),
}
